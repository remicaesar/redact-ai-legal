...
Each gate states its conditions exactly once, as data, and the UI renders that list rather than rebuilding it. `external_llm_gate_policy()` returns `conditions` (all seven, each with `passed`) and derives `failed_conditions` and `policy` from it; `docx_export_conditions()` / `pdf_export_conditions()` in `app.py` are what `get_exportable_docx()` and `pdf_export_blockers()` refuse on, and `export_gate_state()` exposes the same list as `export_gate` on `/api/document/<id>`. Never re-derive a gate in the browser: a JavaScript copy that modelled 5 of the 7 release conditions once put five green ticks and a live download button directly above the gate's own red "Blocked". `tests/test_trust_surfaces.py` pins both halves — that the studio's release panel hardcodes no condition of its own, and that the "This export" panel and the export endpoint agree across a matrix of review states.

The two gates answer different questions and the UI must say which: the reviewed export reflects the reviewer's decisions (a deliberately retained identifier stays in it), while the external-LLM gate is clearance to send the document off the machine. Likewise `residual_risk` is the detection-time measurement the accuracy audit is calibrated on and `post_review_residual_risk` is the level the release gate was decided against — show both, never overwrite the first with the second.
